...
class ListTable : public List {	// —писок
public:
	...
	bool insertElem(Data elem) {
		...
		addToHead(head, elem);		// »зменЄн head

		if (count != getCount())	// »зменение количества элементов
			flag = true;
...
public:
	...
	int hash(Data data) {				//вычисление номера €чейки i, умножение на степень
